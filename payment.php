...
<!-- Bill Section -->
<div class="bill">
    Total Cost: $<span id="total_cost"><?php echo number_format($discounted_total, 2); ?></span> for <span id="days"><?php echo $days; ?></span> days
</div>
...
<div class="form-group coupon-section">
    ...
    <span id="coupon_message" class="coupon-message"></span>
</div>

<script>
    // Configuration
    const validCouponCode = "StayNew"; // Valid coupon code
    ...
    applyCouponButton.addEventListener('click', function () {
        const enteredCoupon = couponInput.value.trim(); // Get entered coupon code
        const originalCost = parseFloat(totalCostElement.textContent); // Extract the current total cost
        let discountedTotal = originalCost; // Start with the current total cost

        // Validate the coupon code
        if (enteredCoupon.toLowerCase() === validCouponCode.toLowerCase()) {
            discountedTotal = originalCost - (originalCost * discountRate); // Apply discount
            couponMessageElement.textContent = "Coupon Applied! 10% discount has been applied.";
            couponMessageElement.style.color = 'green';
            
            // Disable the button after applying the coupon
            applyCouponButton.disabled = true;
            applyCouponButton.style.cursor = 'not-allowed'; // Change cursor to indicate non-clickable

            // Optionally, clear the coupon input field
            couponInput.value = "";
        } else if (enteredCoupon === "") {
            couponMessageElement.textContent = "Please enter a coupon code.";
            couponMessageElement.style.color = 'red';
        } else {
            couponMessageElement.textContent = "Invalid or expired coupon.";
            couponMessageElement.style.color = 'red';
        }

        // Update the total cost on the page
        totalCostElement.textContent = discountedTotal.toFixed(2);
    });
</script>
